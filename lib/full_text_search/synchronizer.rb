module FullTextSearch
  class Synchronizer
    def upsert(record)
      searcher_record =
        SearcherRecord.find_or_initialize_by(searcher_record_keys(record))
    ...
    def destroy(record)
      SearcherRecord.where(searcher_record_keys(record)).destroy_all
    end

    private
    def searcher_record_keys(record)
      case record
      when WikiContent
        {
