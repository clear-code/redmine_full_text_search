...
class AddIsContainerPrivateToFtsTargets < ActiveRecord::Migration[6.1]
  def up
    return unless table_exists?(:fts_targets)

    add_column :fts_targets, :is_container_private, :boolean

    if Redmine::Database.mysql?
      split_journal_private_flags
      add_index :fts_targets, :is_container_private
    else
  ...
  def down
    ...
    if Redmine::Database.mysql?
      remove_index :fts_targets, :is_container_private
    ...
    remove_column :fts_targets, :is_container_private
  end
...
SET fts_targets.is_private = journals.private_notes,
    fts_targets.is_container_private = issues.is_private
WHERE fts_targets.source_type_id = #{journal_type.id}
      SQL
    else
      execute(<<-SQL)
UPDATE fts_targets
SET is_private = journals.private_notes,
    is_container_private = issues.is_private
FROM journals
JOIN issues
  ON journals.journalized_type = 'Issue' AND
     issues.id = journals.journalized_id
